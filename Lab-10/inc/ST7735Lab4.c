...
void ST7735_XYplotInit(char *title, int32_t minX, int32_t maxX, int32_t minY, int32_t maxY, uint16_t bcolor) {
	...
	ST7735_FillRect(0, 3 * 10, ST7735_TFTWIDTH, ST7735_TFTHEIGHT - (3 * 10), bcolor);
	minXG = minX;
	maxXG = maxX;
	minYG = minY;
	maxYG = maxY;
...
void ST7735_XYplot(uint32_t num, int32_t bufX[], int32_t bufY[], uint16_t color) {
    int i = 0;
	for (i = 0; i < num; i++) {
		if (bufX[i] < maxXG && bufX[i] > minXG && bufY[i] < maxYG && bufY[i] > minYG) {
			uint32_t xVal = (bufX[i] - minXG) * ST7735_TFTWIDTH / (maxXG - minXG);
		//uint32_t yVal = ((1 - ((bufY[i] - minYG) / (maxYG - minYG))) * (160-32)) + 32;
		uint32_t yVal = 160 - ((((bufY[i] - minYG) * 128) / (maxYG - minYG)) );
		ST7735_DrawPixel(xVal, yVal, color);
		}
	}
	
	/*
	uint32_t absMinX = 0;
	if (minXG < 0) absMinX = minXG * -1;
	else absMinX = minXG;
	uint32_t absMinY = 0;
	if (minYG < 0) absMinY = minYG * -1;
	else absMinY = minYG;
	for (uint32_t i = 0; i < num; i++) {
		uint32_t xVal = ((((bufX[i] + absMinX) * 1000) / (maxXG - minXG)) * ST7735_TFTWIDTH) / 1000;
		uint32_t yVal = ((((bufY[i] + absMinY) * 1000) / (maxYG - minYG)) * ST7735_TFTHEIGHT) / 1000;
		//ST7735_FillRect(xVal, yVal, (maxXG - minXG) / ST7735_TFTWIDTH, (maxYG - minYG) / ST7735_TFTHEIGHT, color);
		ST7735_DrawPixel(xVal, yVal, color);
	}
	*/
}
