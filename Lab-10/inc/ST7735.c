...
*/
void ST7735_sDecOut2(int32_t n){ //my implementation
	...
	if (n > 9999){
		...
		ST7735_OutChar(hundreds + 48);
		ST7735_OutChar('.');
		ST7735_OutChar(tens + 48);
		ST7735_OutChar(ones + 48);
	}
}
...
*/
void ST7735_uBinOut6(uint32_t n){ // my implementation
	int t = ((n*100) + 32) >> 6;
	...
	if (n > 63999){
		...
		if(tenthousands == 0){
			...
			ST7735_OutChar(tenthousands + 48);
			ST7735_OutChar(thousands + 48);
		}
...
void ST7735_XYplot(uint32_t num, int32_t bufX[], int32_t bufY[], uint16_t color){
  int i = 0;
	while (i < num){
		bool inBoundsOfX = bufX[i] > minimumX && bufX[i] < maximumX;
		bool inBoundsOfY = bufY[i] > minimumY && bufY[i] < maximumY;
		bool inBounds = inBoundsOfX && inBoundsOfY;
		if (inBounds){
			int xCoordinate = (128 * (bufX[i] - minimumX)/(maximumX - minimumX));
			int yCoordinate = (32 + (128 * (maximumY - bufY[i])/(maximumY - minimumY)));
